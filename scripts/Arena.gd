...
## hit pips, and reddens and cracks as it takes damage. That colour split is the
## only thing a player has to learn to read the arena.
...
func _draw_solid(r: Rect2) -> void:
	...
	draw_rect(Rect2(r.position, Vector2(r.size.x, 5.0)), SOLID_TOP)
	draw_line(r.position + Vector2(8.0, 10.0), Vector2(r.end.x - 8.0, r.position.y + 10.0),
		Color(0.58, 0.38, 0.16, 0.42), 1.0)
