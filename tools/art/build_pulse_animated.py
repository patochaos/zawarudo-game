...
SHEETS = (
    SheetSpec("idle-source.png", 4, 1, 649, (("idle", (0, 1, 2, 3)),)),
    SheetSpec("walk-source.png", 3, 2, 360, (("walk", (0, 1, 2, 3, 4, 5)),)),
    SheetSpec("run-source.png", 3, 2, 340, (("run", (0, 1, 2, 3, 4, 5)),)),
    # The airborne keys spread the rigid wings farther than the body. One scale
    # across all four preserves the intended compact-rise/extended-rise change.
    SheetSpec(
        "air-source.png",
        2,
        2,
    ...
    # The first recoil is the upright scale reference. Kneeling keys must stay
    # naturally short instead of being enlarged frame by frame.
    SheetSpec("defeat-source.png", 4, 1, 638, (("defeat", (0, 1, 2, 3)),)),
)
...
def trim_foreground(cell: Image.Image) -> Image.Image:
    ...
    if not largest:
        raise ValueError("Generated cell contains no usable Pulse foreground")
...
def shared_scale(frames: list[Image.Image], reference_body_height: int) -> float:
    """Choose one scale for a whole generated sheet, including short poses."""
...
def save_contact_sheet(output: Path, atlases: dict[str, list[Image.Image]]) -> None:
    ...
    for row, (name, frames) in enumerate(atlases.items()):
        ...
        draw.text((margin, y + 50), name.upper(), fill=(198, 242, 91, 255))
        for index, frame in enumerate(frames):
            ...
            draw.text((x + 6, y + 5), str(index + 1), fill=(191, 199, 207, 255))
    sheet.save(output / "contact-sheet.png")


def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize generated Pulse animation sheets.")
