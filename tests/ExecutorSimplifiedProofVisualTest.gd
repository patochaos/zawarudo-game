...
const DISPLAY_SCALE := Vector2(0.25, 0.25)
...
class ProofStage extends Node2D:
	func _draw() -> void:
		...
		draw_circle(Vector2(930.0, 325.0), 112.0, Color(0.74, 0.58, 0.29, 0.085))
...
func _capture() -> void:
	...
	if image.is_empty():
		push_error("Could not load simplified Executor atlas")
		quit(1)
		return
	...
	for index in 4:
		...
		sprite.position = Vector2(x_positions[index], 592.0)
		stage.add_child(sprite)

	var title := Label.new()
	title.text = "ARENA SPRITE PROOF  /  56 PX  /  FIVE FLAT COLOR FAMILIES"
